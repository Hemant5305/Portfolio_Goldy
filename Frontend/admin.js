/* =========================================================
   DEMO STATE  (in-memory — same shape/content as index.html)
========================================================= */
const state = {
    services: [
    ...
    // sample bookings so the demo dashboard has data to show off
    bookings: [
...
document.getElementById('otpForm').addEventListener('submit', function (e) {
    ...
    if (entered.length === 6 && entered === currentOtp) {
        ...
        document.getElementById('adminShell').classList.remove('hidden-panel');
        resetLoginFlow();
        renderAdminData();
...
function logoutDemo(e) {
    e.preventDefault();
    document.getElementById('adminShell').classList.add('hidden-panel');
...
function switchTab(tab, el) {
    ...
    document.querySelectorAll('.admin-tab').forEach(t => { t.classList.remove('show'); t.classList.add('hidden-panel'); });
...
function renderAdminData() {
  ...
  `).join('') || `<tr><td colspan="5" class="empty-note">No booking requests yet — new bookings from the site will appear here instantly.</td></tr>`;
    ...
    // activity feed (derived from bookings + review queue, most recent first)
    const activity = [
    ...
    <div class="activity-item"><span class="activity-dot"></span><div><div>${a.text}</div><div class="subtle">${a.when}</div></div></div>
...
function removeGalleryItem(i) { state.gallery.splice(i, 1); renderAdminData(); showToast('Photo removed from gallery'); }
function removeVideoItem(i) { state.videos.splice(i, 1); renderAdminData(); showToast('Video removed'); }
function updateServicePrice(i, val) { state.services[i].price = parseInt(val) || 0; showToast(`${state.services[i].name} price updated to ₹${state.services[i].price}`); }
function removeService(i) { const name = state.services[i].name; state.services.splice(i, 1); renderAdminData(); showToast(`${name} removed from menu`); }
function setReviewStatus(i, status) { state.reviews[i].status = status; renderAdminData(); showToast(`Review by ${state.reviews[i].name} ${status.toLowerCase()}`); }
...
/* =========================================================
   INIT — demo opens straight on the staff login screen
========================================================= */
resetLoginFlow();